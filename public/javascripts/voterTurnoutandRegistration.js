...
(function(){	
	...
	var color = d3.scale.ordinal()
		.range(["#FFA500", "#40C8FF"]); // Bar Colors
	...
	var svg = d3.select("article").append("svg") /* Location of the Graph */
		.attr("width", width + margin.left + margin.right)
		.attr("height", height + margin.top + margin.bottom)
		.append("g")
		.attr("transform", "translate(" + margin.left + "," + adjusted + ")"); /* adjusted replaced margin.top to shift graph up by 10px */

	d3.csv("../data/dataVoterTurnout.csv", function(error, data) { /* Data Files */
		...
		y.domain([0, d3.max(data, function(d) { return 100 })]);  /* Scale of Chart MAX number */
		...
		svg.append("g")
			...
			.attr("y", 6)
			.attr("dy", ".71em") /* Distance of Text to Vertical Line */
			.style("text-anchor", "end")
			.text("Voter (%)");  /* TEXT of Left */
...
(function(){	
	...
	var yAxis = d3.svg.axis()
		...
		var svg = d3.select("article2").append("svg")
			...
			.append("g")
			.attr("transform", "translate(" + margin.left + "," + adjusted + ")"); /* adjusted replaced margin.top to shift graph up by 10px */
		d3.csv("../data/dataVoterRegistration.csv", function(error, data) {
			var ageNames = d3.keys(data[0]).filter(function(key) { return key !== "State"; });
...
(function(){
        ...
        var dataCSV = "../data/dataVoterTurnoutBar.csv";
...
(function(){
        ...
        //start CSV for bar charts
        var dataCSV = "../data/dataVoterRegistrationBar.csv";
